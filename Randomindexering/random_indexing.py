...
class RandomIndexing(object):
    ...
    def clean_line(self, line):
        ...
        return "".join(new_char_list).split()

    ##
    ## @brief      A generator function providing
    ...
    def create_word_vectors(self):
        ...
        for word in self.__vocab:
            start = [0]*2000
            self.__cv[word] = start
            random_index = random.sample(range(0, self.__dim), self.__non_zero)
            for i in random_index:
                start[i] = random.choice(self.__non_zero_values)
            self.__rv[word] = start
        ...
        for line in self.text_gen():
            for word in line:
                ...
                for j in range(1, self.__lws):
                    print("test1")
                    if line.index(word)-j >= 0:
                        self.__cv[word] += self.__rv[line[line.index(word)-j]]
                    else:
                        break
                for k in range(1, self.__rws):
                    print("test2")
                    if line.index(word)+k <= len(line)-1:
                        self.__cv[word] += self.__rv[line[line.index(word)+k]]
                    else:
                        break
    ##
    ...
    def find_nearest(self, words, k=5, metric='cosine'):
        ...
        test = []
        for i in words:
            test.append(self.get_word_vector(i))
        print(neigh.fit(test))
        return neigh.fit(test)
