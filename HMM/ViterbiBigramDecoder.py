...
class ViterbiBigramDecoder(object):
    ...
    def viterbi(self, s):
        ...
        for t in range(1, len(index)):
            letter = index[t]  # inmatad bokstav
            D = np.add(self.v[t-1], self.a[:, letter])
            max_index = list(D).index(max(D))
            self.v[t, letter] = max(D) + self.b[letter, letter]  # Chansen att man klickar rätt  - Sparar maximala sannolikheten
            self.backptr[t, letter] = max_index  # Sparar varifrån vi gick för att få max
            
            for key in Key.neighbour[letter]:
                key_index = Key.char_to_index(key)
                D = np.add(self.v[t-1], self.a[:, key_index])
                max_index = list(D).index(max(D))
                self.v[t, key_index] = max(D) + self.b[letter, key_index]
                self.backptr[t, key_index] = max_index


        print_list = [Key.START_END]
        for i in range(len(s)-1, 0, -1):
            print_list.append(self.backptr[i, print_list[len(print_list) -1]])
        print_list.reverse()
        result = ''
        for j in print_list[:-1]:
