...
class BigramTester(object):
    ...
    def compute_entropy_cumulatively(self, word):
        ...
        if self.last_index != -1:
            try:
                sum += math.log((self.lambda1*float(self.bigram_prob[self.last_index][word])) + self.lambda2*(int(self.unigram_count[word])/self.total_words) + self.lambda3)
            except KeyError:
                try:
                    sum += math.log(self.lambda2*(int(self.unigram_count[word])/self.total_words) + self.lambda3)
                except KeyError:
                    sum += math.log(self.lambda3)
        else:
            try:
                sum += math.log(self.lambda2*(int(self.unigram_count[word])/self.total_words) + self.lambda3)
            except KeyError:
        ...
        self.logProb = self.logProb - (sum/len(self.tokens))
